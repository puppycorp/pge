...
impl pge::App for FpsShooter {
    fn on_create(&mut self, state: &mut State) {
        ...
        plane_node.collision_shape = Some(CollisionShape::new(glam::Vec3::new(
            plane_size, 0.1, plane_size,
        )));
    ...
    fn on_process(&mut self, state: &mut State, delta: f32) {
        ...
        self.hit_log
            .retain(|(_, bullet_id)| bullet_ids.contains(bullet_id));
        for orc in &self.orcs {
            let node = match state.nodes.get(&orc.node) {
                Some(node) => node,
                None => continue,
        ...
        for orc in &self.orcs {
            ...
            if clamped.y < 0.0
                || clamped.y > 200.0
                || clamped.x.abs() > bounds
                || clamped.z.abs() > bounds
            {
                panic!(
                    "Orc out of bounds: id={:?}, pos={:?}, bounds=±{}, y=[0,200]",
                    orc.node, clamped, bounds
                );
            }
        }

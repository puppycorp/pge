...
impl DrawRect {
    pub fn generate_vertices_indices(&self) -> (Vec<[f32; 2]>, Vec<u16>) {
        ...
        builder.begin(point(
            self.top_left[0] + self.top_left_radius,
            self.top_left[1],
        ));

        // Top line and top right corner
        builder.line_to(point(
            self.top_right[0] - self.top_right_radius,
            self.top_right[1],
        ));
        builder.cubic_bezier_to(
            point(
                self.top_right[0] - self.top_right_radius,
                self.top_right[1] + self.top_right_radius,
            ),
        ...
        builder.line_to(point(
            self.bottom_right[0],
            self.bottom_right[1] - self.bottom_right_radius,
        ));
        builder.cubic_bezier_to(
            point(
                self.bottom_right[0] - self.bottom_right_radius,
                self.bottom_right[1] - self.bottom_right_radius,
            ),
        ...
        builder.line_to(point(
            self.bottom_left[0] + self.bottom_left_radius,
            self.bottom_left[1],
        ));
        builder.cubic_bezier_to(
            point(
                self.bottom_left[0] + self.bottom_left_radius,
                self.bottom_left[1] - self.bottom_left_radius,
            ),
        ...
        builder.line_to(point(
            self.top_left[0],
            self.top_left[1] + self.top_left_radius,
        ));
        builder.cubic_bezier_to(
            point(
                self.top_left[0] + self.top_left_radius,
...
impl Lineariser {
    ...
    fn inner_linearize(&mut self, item: &GUIElement, outline: Option<Outline>) {
        ...
        if item.children.len() > 0 {
            match item.flex_dir {
                Flex::Horizontal => {
                    let total_grow: f32 = item
                        .children
                        .iter()
                        .map(|child| child.grow.max(1) as f32)
                        .sum();
        ...
        for draw in &items {
            match draw {
                DrawItem::Rect(rect) => {
                    ...
                    self.positions
                        .extend(vertices.iter().map(|&p| [p[0], p[1], 0.0]));
                    let adjusted_indices: Vec<u16> =
                        indices.iter().map(|&i| i + current_offset).collect();
                    ...
                    self.colors
                        .extend(std::iter::repeat(rect.background_color).take(vertices.len()));
...
mod tests {
    ...
    #[test]
    fn test_rectangle() {
        let rect = rect().background_color(Color::RED);
    ...
    fn test_text_rendering() {
        let vstack = column(&[text("row 1"), text("row 2"), text("row 3")]);

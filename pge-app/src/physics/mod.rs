...
fn calculate_impulse(
    ...
    let node1_inv_mass = if node1.physics.mass == 0.0 {
        0.0
    } else {
        1.0 / node1.physics.mass
    };
    let node2_inv_mass = if node2.physics.mass == 0.0 {
        0.0
    } else {
        1.0 / node2.physics.mass
    };
    ...
    let node1_inertia_tensor = if node1.inertia_tensor() == glam::Mat3::ZERO {
        glam::Mat3::ZERO
    } else {
        node1.inertia_tensor().inverse()
    };
    let node2_inertia_tensor = if node2.inertia_tensor() == glam::Mat3::ZERO {
        glam::Mat3::ZERO
    } else {
        node2.inertia_tensor().inverse()
    };
    let term_a = collision
        .normal
        .dot(node1_inertia_tensor * (r1.cross(collision.normal)).cross(r1));
    let term_b = collision
        .normal
        .dot(node2_inertia_tensor * (r2.cross(collision.normal)).cross(r2));
    ...
    let tangent_impulse = if jt.abs() < max_friction {
        jt * tangent
    } else {
        max_friction * tangent * jt.signum()
    };
        ...
        while dt > 0.0 && iterations < max_iterations {
            ...
            for collision in &self.broad_phase_collisions {
                let (node1, node2) = match (
                    state.nodes.get(&collision.node1),
                    state.nodes.get(&collision.node2),
                ) {
                    (Some(node1), Some(node2)) => (node1, node2),
                    _ => continue,
                };

                if node1.physics.typ == PhycisObjectType::Static
                    && node2.physics.typ == PhycisObjectType::Static
                {
                    continue;
                }
                ...
                if self
                    .collision_cache
                    .contains(&(collision.node1, collision.node2))
                {
            ...
            if let Some(collision) = earliest_collision {
                ...
                let time_step = if earliest_toi < min_dt {
                    min_dt
                } else {
                    earliest_toi
                };
                ...
                self.collision_cache
                    .insert((collision.node1, collision.node2));
            } else {

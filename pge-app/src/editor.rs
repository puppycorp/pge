...
impl SceneViewer {
    fn new(state: &mut State, scene_id: ArenaId<Scene>, settings: &EditorSettings) -> Self {
        ...
        if settings.add_light {
            ...
            light_node.translation =
                center + Vec3::new(0.0, light_distance * 0.5, -light_distance * 0.5);
    ...
    fn on_process(&mut self, state: &mut State, dt: f32) {
        ...
        if orbit_controls_active {
            ...
            if rebind_orbit {
                self.orbit_controller.set_from_target_and_position(
        ...
        let rotating_camera =
            self.rotate_left || self.rotate_right || self.rotate_up || self.rotate_down;
    ...
    fn on_mouse_input(&mut self, event: MouseEvent) {
        match event {
            ...
            MouseEvent::Pressed { button } => match button {
                MouseButton::Right => {
            ...
            MouseEvent::Released { button } => match button {
                MouseButton::Right => {
...
impl EditorPlugin {
    ...
    fn discover_new_scenes(&mut self, state: &State) -> Vec<ArenaId<Scene>> {
        ...
        for (scene_id, _) in state.scenes.iter() {
            ...
            let has_nodes = state.nodes.iter().any(|(_, node)| {
                node.parent == NodeParent::Scene(scene_id) || node.scene_id == Some(scene_id)
            });
            if !has_nodes {
                continue;
            }
    ...
    pub fn on_mouse_input(&mut self, window_id: ArenaId<Window>, event: MouseEvent) {

        ...
        Some(gltf_mesh) => {
            ...
            for p in gltf_mesh.primitives() {
                ...
                if let Some(iter) = reader.read_tex_coords(0) {
                    ...
                    log::warn!(
                        "Primitive #{} is missing texture coordinate data",
                        p.index()
                    );
                }
...
pub fn load_animation(
    ...
    for channel in anim.channels() {
        ...
        let target_node_id = match parser_state.node_map.get(&target_node.index()) {
            // Use parser_state
            Some(id) => id,
            None => {
                log::warn!(
                    "Animation target node not found: {}",
                    target_node.name().unwrap_or_default().to_string()
                );
                continue;
            }
        };
        ...
        let output = match reader.read_outputs().unwrap() {
            ...
            ReadOutputs::Rotations(output) => AnimationOutput::Rotation(match output {
                Rotations::I8(d) => d
                    .map(|o| {
                        Quat::from_array([
                            o[0] as f32 / 127.0,
...
pub fn load_gltf<P: AsRef<Path>>(p: P, state: &mut State) -> Model3D {
    ...
    for gltf_material in document.materials() {
        ...
        if let Some(base_color_texture) = pbr.base_color_texture() {
            ...
            let pbr_texture_id = match parser_state
                .texture_map
        ...
        if let Some(metallic_roughness_texture) = pbr.metallic_roughness_texture() {
            crate::log2!(
                ...
                metallic_roughness_texture.texture().index()
            );
            let texture_id = match parser_state
                .texture_map
        ...
        if let Some(emissive_texture) = gltf_material.emissive_texture() {
            ...
            let texture_id = match parser_state
                .texture_map
                .get(&emissive_texture.texture().index())
            {
                Some(id) => id,
                None => {
                    continue;
        ...
        parser_state
            .material_map
            .insert(material_index, material_id);
    }

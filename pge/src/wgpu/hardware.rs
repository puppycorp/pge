    ...
    H: Hardware,
{
    fn new(
        ...
        let progress_interval = max_iterations.map(iteration_log_interval).unwrap_or(0);
    ...
    fn on_window_resized(&mut self, window_id: winit::window::WindowId, size: PhysicalSize<u32>) {
        let Some(window_index) = self
            .windows
            .iter()
            .position(|window_ctx| window_ctx.winit_id == window_id)
        else {
        ...
        match event {
            UserEvent::CreateWindow(args) => {
                let mut window_attributes =
                    winit::window::Window::default_attributes().with_title(&args.name);
                ...
                if let Some(size) = args.size {
                    window_attributes.inner_size = Some(winit::dpi::Size::Physical(
            ...
            UserEvent::CreatePipeline {
                ...
                let point_light_bind_group_layout =
                    RawPointLight::create_bind_group_layout(&self.device);
                let base_texture_bind_group_layout =
                    TextureBuffer::create_bind_group_layout(&self.device);
                let metallic_roughness_texture_bind_group_layout =
                    TextureBuffer::create_bind_group_layout(&self.device);
                let normal_texture_bind_group_layout =
                    TextureBuffer::create_bind_group_layout(&self.device);
                let occlusion_texture_bind_group_layout =
                    TextureBuffer::create_bind_group_layout(&self.device);
                let emissive_texture_bind_group_layout =
                    TextureBuffer::create_bind_group_layout(&self.device);
                ...
                let (render_pipeline, depth_texture_view, uses_depth) = if name == "gui" {
                    ...
                    let shader_source =
                        wgpu::ShaderSource::Wgsl(include_str!("../shaders/3d_shader.wgsl").into());
                    ...
                    let depth_texture_view = Arc::new(
            ...
            UserEvent::Render { window, encoder } => {
                ...
                if screenshot_path.is_none() {
                    if let Some(dir) = &self.screenshot_dir {
                        if self.screenshot_interval > 0
                            && frame_index % self.screenshot_interval == 0
                        {
                ...
                let window_ctx = match self
                    .windows
                    .iter()
                    .find(|window_ctx| window_ctx.window_id == window.id)
                {
                    Some(window) => window,
                    None => {
                ...
                for (pass_index, pass) in encoder.passes.into_iter().enumerate() {
                    ...
                    let pipeline_ctx = match self
                        .pipelines
                        .iter()
                        .find(|pipeline_ctx| pipeline_ctx.id == pipeline.id)
                    {
                    ...
                    let mut wgpu_pass =
                        wgpu_encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    ...
                    for subpass in &pass.subpasses {
                        for (slot, texture) in &subpass.textures {
                            let texture_ctx =
                                match self.textures.iter().find(|t| t.id == texture.id) {
                        ...
                        for (slot, buffer) in &subpass.vertex_buffers {
                            let buffer_ctx =
                                match self.buffers.iter().find(|b| b.id == buffer.handle.id) {
                                    Some(buffer) => buffer,
                                    None => {
                                        log::error!("Buffer not found: {:?} => RETURN", buffer);
                                        return;
                            ...
                            wgpu_pass.set_vertex_buffer(
                                *slot,
                                buffer_ctx.buffer.slice(buffer.range.clone()),
                            );
                        }
                        if let Some(slice) = &subpass.index_buffer {
                            let buffer_ctx = match self
                                .buffers
                                .iter()
                                .find(|b| b.id == slice.handle.id)
                            {
                                Some(buffer) => buffer,
                                None => {
                            ...
                            wgpu_pass.set_index_buffer(
                                buffer_ctx.buffer.slice(slice.range.clone()),
                ...
                if let (
                    Some(path),
                    Some(buffer),
                    Some((width, height, unpadded_bytes_per_row, padded_bytes_per_row)),
                ) = (screenshot_path, screenshot_buffer, screenshot_info)
                {
                    ...
                    match rx.recv() {
                        ...
                        Err(err) => {
                            log::error!(
                                "Failed to receive screenshot buffer map result: {:?}",
                                err
                            );
                            buffer.unmap();
                            return;
            ...
            UserEvent::DestroyBuffer { buffer_id } => {
            ...
            UserEvent::WriteBuffer { buffer, data } => {
            ...
            UserEvent::SaveScreenshot { window, path } => {
    ...
    fn about_to_wait(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
        ...
        if let Some(max) = self.max_iterations {
            if self.progress_interval > 0
                && (self.iterations % self.progress_interval == 0 || self.iterations == max)
            {
...
fn screenshot_interval_from_env() -> u64 {
    match env::var("SCREENSHOT_INTERVAL")
        .ok()
...
fn run_headless_loop<A, H, E, F>(
    app: A,
    hardware: H,
    mut tick: F,
    mut app_events: Option<mpsc::Receiver<E>>,
) -> anyhow::Result<()>
where
    A: App<E>,
    H: Hardware,
    ...
    A: App<E>,
{
    ...
    run_headless_loop(
        app,
        MockHardware::new(),
        |engine, dt| engine.tick_headless(dt),
        app_events,
    )
}
    ...
    A: App<E>,
{
    ...
    let (device, queue) = match block_on(adapter.request_device(
        ...
        Err(err) => {
            log::error!(
                "Failed to create headless device ({err}); falling back to mock software renderer."
            );
            return run_headless_with_mock(app, app_events);
        }
    };
...
impl Hardware for WgpuHardware {
    ...
    fn render(&mut self, encoder: RenderEncoder, window: WindowHandle) {
        self.proxy.send_event(UserEvent::Render { window, encoder });
    }
...
impl Hardware for HeadlessWgpuHardware {
    ...
    fn render(&mut self, encoder: RenderEncoder, window: WindowHandle) {
        ...
        for (pass_index, pass) in encoder.passes.into_iter().enumerate() {
            ...
            for subpass in &pass.subpasses {
                ...
                for (slot, buffer) in &subpass.vertex_buffers {
                    ...
                    wgpu_pass
                        .set_vertex_buffer(*slot, buffer_ctx.buffer.slice(buffer.range.clone()));
                }
                if let Some(slice) = &subpass.index_buffer {
        ...
        if screenshot_path.is_some() {
            ...
            screenshot_buffer = Some(output_buffer);
            screenshot_info = Some((
                window_ctx.size.width,
                window_ctx.size.height,
                unpadded_bytes_per_row,
                padded_bytes_per_row,
            ));
        }
    ...
    fn destroy_window(&mut self, handle: WindowHandle) {
        self.windows
            .retain(|window_ctx| window_ctx.window_id != handle.id);
    }

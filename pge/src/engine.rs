    ...
    H: Hardware,
{
    pub fn new(mut app: A, mut hardware: H) -> Self {
        ...
        hardware.write_buffer(
            default_material,
            bytemuck::cast_slice(&[default_material_data]),
        );
    ...
    fn process_nodes(&mut self) {
        ...
        if sort_timer.elapsed() > Duration::from_millis(10) {
            crate::log3!(
                "Topo sort {} nodes took {:?}",
                self.topo_sorted_nodes.len(),
                sort_timer.elapsed()
            );
        }
    ...
    fn process_textures(&mut self) {
        for (texture_id, texture) in &self.state.textures {
            ...
            match &texture.source {
                ...
                TextureSource::None => {
                    log::warn!("TextureSource::None encountered - using white texture");
                }
            };
            let handle = self
                .hardware
                .create_texture(&texture.name, &data, width, height);
    ...
    fn process_materials(&mut self) {
        for (material_id, material) in &self.state.materials {
            ...
            let buffer = self
                .hardware
                .create_buffer(&format!("material_buffer_{:?}", material_id.index()), 1000);
            self.hardware
                .write_buffer(buffer, bytemuck::bytes_of(&raw_material));
                ...
                if primitive.topology == PrimitiveTopology::TriangleList {
                    ...
                    if primitive.tex_coords.len() > 0 {
                        self.tex_coords_buffer
                            .write(bytemuck::cast_slice(&primitive.tex_coords));
                    } else {
                        let tex_coords = vec![[0.0, 0.0]; primitive.vertices.len()];
                        self.tex_coords_buffer
                            .write(bytemuck::cast_slice(&tex_coords));
                    }
                    ...
                    for node_id in node_ids {
                        ...
                        let buffer =
                            self.scene_instance_buffers
                                .entry(scene_id)
                                .or_insert_with(|| {
                                    Buffer::new(self.hardware.create_buffer(
                                        &format!("instances_{:?}", scene_id.index()),
                                        1000,
                                    ))
                                });

                        let instance_start =
                            buffer.len() as u32 / std::mem::size_of::<RawInstance>() as u32;
    ...
    fn process_cameras(&mut self) {
        for (cam_id, cam) in &self.state.cameras {
            ...
            let camera_position = node.global_transform.w_axis.truncate();
            ...
            let buffer = self.camera_buffers.entry(cam_id).or_insert_with(|| {
                Buffer::new(
                    self.hardware
                        .create_buffer(&format!("camera_buffer_{:?}", cam_id.index()), 1000),
                )
            });
    ...
    fn process_point_lights(&mut self) {
        let mut scene_light_map: std::collections::HashMap<ArenaId<Scene>, Vec<RawPointLight>> =
            HashMap::new();
        ...
        for (scene_id, scene_lights) in scene_light_map {
            ...
            self.point_light_buffers
                ...
                .or_insert_with(|| {
                    crate::log2!(
                        "Creating new point light buffer for scene ID: {:?}",
                        scene_id
                    );
                    Buffer::new(self.hardware.create_buffer("pointlight", 1000))
                })
                .write(bytemuck::bytes_of(&scene_light_buffer));
        }
    ...
    fn process_ui(&mut self) {
        for (ui_id, gui) in &self.state.guis {
            let c: &mut Compositor =
                self.ui_compositors.entry(ui_id.clone()).or_insert_with(|| {
                    crate::log2!("Creating new Compositor for UI ID: {:?}", ui_id); // Debug print
                    Compositor::new()
                });
            c.process(gui);

            let buffers = self.gui_buffers.entry(ui_id).or_insert_with(|| {
    ...
    pub fn on_mouse_input(&mut self, window: WindowHandle, event: MouseEvent) {
        ...
        self.app
            .on_mouse_input(window_ctx.window_id, event, &mut self.state);
    }
    ...
    pub fn render(&mut self, dt: f32) {
        ...
        for (window_id, _) in &self.state.windows {
            ...
            let args = match self.get_window_render_args(window_id) {
                ...
                None => {
                    log::error!(
                        "Window render args not found for {:?}, skipping render.",
                        window_id
                    );
                    continue;
                }
                ...
                for call in calls {
                    ...
                    if let Some(material_id) = call.material {
                        if let Some(material_buffer) = self.state.materials.get(&material_id) {
                            ...
                            if let Some(metallic_roughness_texture_id) =
                                material_buffer.metallic_roughness_texture
                            {
                    ...
                    pass.set_vertex_buffer(
                        3,
                        self.tex_coords_buffer.slice(call.tex_coords.clone()),
                    );
                    ...
                    pass.draw_indexed(
                        call.indices_range.clone(),
                        instances.start as u32..instances.end as u32,
                    );
                }
            }
            ...
            if let Some(gui_buffers) = self.gui_buffers.get(&args.ui) {
                    ...
                    && gui_buffers.index_range.start != gui_buffers.index_range.end
                {
                    ...
                    gui_pass.set_vertex_buffer(
                        ...
                        gui_buffers
                            .vertices_buffer
                            .slice(gui_buffers.position_range.clone()),
                    );
                    gui_pass.set_vertex_buffer(
                        1,
                        gui_buffers
                            .color_buffer
                            .slice(gui_buffers.colors_range.clone()),
                    );
                    gui_pass.set_index_buffer(
                        gui_buffers
                            .index_buffer
                            .slice(gui_buffers.index_range.clone()),
                    );
